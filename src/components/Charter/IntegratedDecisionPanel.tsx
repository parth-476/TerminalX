...
export const IntegratedDecisionPanel: React.FC<Props> = ({ lane, ports, vessels, quantityMt, laycanDays }) => {
  ...
  return <>
    <section className="border border-[#F27D26] bg-[#120d08] p-3 mt-3">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-2 mb-3"><Metric label="ACTION" value={decision.action} className={actionClass}/><Metric label="SCORE" value={`${decision.score.toFixed(0)}/100`}/><Metric label="CONFIDENCE" value={`${decision.confidence}%`}/><Metric label="FREIGHT" value={<>{trendIcon}{trend}</>}/><Metric label="PORT 14D" value={`${decision.congestion.forecast14d.toFixed(0)}/100`}/><Metric label="TOP VESSEL" value={top ? top.vessel.name : 'NO MATCH'}/></div>
...
const Metric: React.FC<{label:string;value:React.ReactNode;className?:string}> = ({label,value,className='text-white'}) => <div className="border border-[#333] bg-[#080808] p-2 min-h-[58px]"><div className="text-[8px] text-gray-500">{label}</div><div className={`font-bold mt-1 text-[10px] flex items-center gap-1 ${className}`}>{value}</div></div>;
