...
export const TerminalHeader: React.FC<TerminalHeaderProps> = ({ currentView, onViewChange, theme, onThemeChange, assets, onSelectTicker, soundEnabled, onToggleSound }) => {
  ...
  useEffect(() => {
    const key = (e: KeyboardEvent) => {
      const map: Record<string, TerminalViewId> = { F1: 'WORKSPACE', F2: 'FRGT', F3: 'MARKET', F4: 'FCST', F5: 'PORT', F6: 'VSL', F7: 'NEWS', F8: 'OMS', F9: 'AI' };
  ...
  const commands: { id: TerminalViewId; label: string }[] = [
    { id: 'WORKSPACE', label: 'DESK [SIH COMMAND]' }, { id: 'FRGT', label: 'FRGT [GLOBAL LANES]' },
    { id: 'MARKET', label: 'MKT [COMMODITIES]' }, { id: 'FCST', label: 'FCST [FREIGHT FORECAST]' },
    { id: 'PORT', label: 'PORT [INDIA INTEL]' }, { id: 'VSL', label: 'VSL [VESSEL INTEL]' },
    { id: 'NEWS', label: 'NEWS [TRADE/ECO]' }, { id: 'OMS', label: 'OMS [CHARTER EXEC]' }, { id: 'AI', label: 'ASK [AI ANALYST]' }
  ];

  const submit = (e: React.FormEvent) => {
    e.preventDefault(); const raw = commandInput.trim().toUpperCase(); if (!raw) return;
    const aliases: Record<string, TerminalViewId> = { DESK:'WORKSPACE', WORKSPACE:'WORKSPACE', FRGT:'FRGT', SHIP:'FRGT', MAP:'FRGT', MARKET:'MARKET', MKT:'MARKET', FCST:'FCST', FORECAST:'FCST', PORT:'PORT', PORTS:'PORT', VSL:'VSL', VESSEL:'VSL', VESSELS:'VSL', NEWS:'NEWS', OMS:'OMS', AI:'AI', ASK:'AI', ANLY:'FCST', CHAT:'VSL', XL:'NEWS' };
    const id = aliases[raw.replace(' <GO>','')];
    if (id) onViewChange(id);
    else { const found = assets.find(a => a.ticker.toUpperCase().includes(raw) || a.name.toUpperCase().includes(raw)); if (found) { onSelectTicker(found.ticker); onViewChange('MARKET'); } }
