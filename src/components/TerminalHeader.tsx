...
export const TerminalHeader: React.FC<TerminalHeaderProps> = ({ currentView, onViewChange, theme, onThemeChange, assets, onSelectTicker, soundEnabled, onToggleSound }) => {
  ...
  useEffect(() => {
    const key = (e: KeyboardEvent) => {
      const map: Record<string, TerminalViewId> = { F1:'WORKSPACE', F2:'FRGT', F3:'MARKET', F4:'FCST', F5:'PORT', F6:'VSL', F7:'PROC', F8:'CHART', F9:'DATA', F10:'AI' };
  ...
  const commands: { id: TerminalViewId; label: string }[] = [
    ...
    { id:'PROC', label:'PROC [CARGO PROCUREMENT]' }, { id:'CHART', label:'CHART [CHARTER DECISION]' }, { id:'DATA', label:'DATA [LIVE FEEDS]' }, { id:'AI', label:'ASK [AI ANALYST]'}
  ];
  const submit = (e: React.FormEvent) => {
    e.preventDefault(); const raw = commandInput.trim().toUpperCase(); if (!raw) return;
    const aliases: Record<string, TerminalViewId> = { DESK:'WORKSPACE',WORKSPACE:'WORKSPACE',FRGT:'FRGT',SHIP:'FRGT',MAP:'FRGT',MARKET:'MARKET',MKT:'MARKET',FCST:'FCST',FORECAST:'FCST',PORT:'PORT',PORTS:'PORT',VSL:'VSL',VESSEL:'VSL',VESSELS:'VSL',PROC:'PROC',PROCUR:'PROC',PROCUREMENT:'PROC',NEWS:'NEWS',CHART:'CHART',CHARTER:'CHART',OMS:'CHART',DATA:'DATA',FEED:'DATA',FEEDS:'DATA',AI:'AI',ASK:'AI',ANLY:'FCST',CHAT:'VSL',XL:'NEWS' };
