...
export const SIHCommandCenterView: React.FC<SIHCommandCenterViewProps> = ({ freightLanes, ports, vessels, onNavigateView }) => {
  ...
  const chartHistory = forecast.history.slice(-18).map(item => ({ label: item.date.slice(5), value: item.rate, kind: 'actual' as const }));
    ...
    <div className="max-w-[1600px] mx-auto">
      ...
      <div className="grid xl:grid-cols-[minmax(0,1.7fr)_minmax(350px,0.8fr)] gap-3 mb-3">
        ...
        <section ref={decisionRef} className={`border bg-[#070707] transition-all duration-200 ${focus === 'decision' ? 'border-[#F27D26] shadow-[0_0_30px_rgba(242,125,38,0.08)]' : 'border-[#333]'}`}>
          ...
          <div className="p-4">
            ...
            <div className="mt-4 h-2 bg-[#181818] overflow-hidden"><div className="h-full bg-[#F27D26]" style={{ width: `${integrated.score}%` }}/></div><div className="flex justify-between text-[8px] text-gray-600 mt-1"><span>WAIT</span><span>WATCH</span><span>SPLIT</span><span>FIX NOW</span></div>
      ...
      <div className="grid lg:grid-cols-3 gap-3 mb-3">
        ...
        <SignalPanel title="PROCUREMENT TIMING" icon={<PackageSearch/>} focus={focus === 'procurement'} onClick={() => jumpTo('procurement')}>
          <div className="flex items-end justify-between"><div><div className={`text-xl font-black ${procurement.action === 'BUY NOW' ? 'text-red-400' : procurement.action === 'WAIT' ? 'text-[#00FF41]' : 'text-[#F27D26]'}`}>{procurement.action}</div><div className="text-[8px] text-gray-500 mt-1">MODELLED BUYING WINDOW</div></div><div className="text-right"><div className="text-white text-xs font-bold">{procurement.buyNowQuantityMt.toLocaleString()} MT</div><div className="text-[8px] text-gray-500">BUY NOW</div></div></div><div className="grid grid-cols-2 gap-2 mt-4"><MiniStat label="WAIT" value={`${procurement.waitQuantityMt.toLocaleString()} MT`}/><MiniStat label="LANDED FREIGHT" value={`$${procurement.landedFreightCostUsd.toLocaleString(undefined, { maximumFractionDigits: 0 })}`}/></div><button onClick={() => onNavigateView('PROC')} className="w-full mt-3 border border-[#333] hover:border-[#F27D26] text-[#F27D26] py-2 text-[9px] font-bold">OPEN PROCUREMENT →</button>
        </SignalPanel>
      </div>
      ...
      <footer className="flex flex-wrap justify-between gap-2 text-[8px] text-gray-600 py-2"><span>DEMO / DECISION-SUPPORT DATA • NO LIVE BROKER QUOTE OR CHARTER EXECUTION</span><span>Inputs update the forecast view, vessel screen, port pressure, procurement and integrated decision.</span></footer>
    </div>
  </div>;
};
