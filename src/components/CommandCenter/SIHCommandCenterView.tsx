...
import { Anchor, ArrowRight, BrainCircuit, Gauge, PackageSearch, Ship, TrendingDown, TrendingUp } from 'lucide-react';
    ...
    <div className="h-full overflow-auto bg-black text-[#d1d1d1] font-mono p-3">
      ...
      <div className="grid lg:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)] gap-3 mb-3">
        <section className="border border-[#333] bg-[#080808] p-3">
          <div className="text-[#F27D26] text-xs font-bold mb-3">TODAY'S DECISION — {lane.code}</div>
          <div className="grid md:grid-cols-2 gap-3">
            <DecisionCard title="FREIGHT" value={forecast.direction === 'UP' ? 'LOCK EXPOSURE' : forecast.direction === 'DOWN' ? 'DEFER COMMITMENT' : 'MONITOR'} detail={`7D $${forecast.forecast7d.toFixed(0)} • 14D $${forecast.forecast14d.toFixed(0)} • 30D $${forecast.forecast30d.toFixed(0)}`} onClick={() => onNavigateView('FCST')} />
            <DecisionCard title="PROCUREMENT" value={procurement.action} detail={`${procurement.buyNowQuantityMt.toLocaleString()} MT buy now • ${procurement.waitQuantityMt.toLocaleString()} MT wait`} onClick={() => onNavigateView('PROC')} />
            <DecisionCard title="CHARTER" value={charter?.decision ?? 'WATCH'} detail={charter?.rationale[0] ?? 'No charter signal'} onClick={() => onNavigateView('CHART')} />
            <DecisionCard title="VESSEL" value={topVessel ? topVessel.vessel.name : 'NO MATCH'} detail={topVessel ? `Match ${topVessel.score}/100 • ${topVessel.vessel.dwt.toLocaleString()} DWT` : 'Capacity screen returned no match'} onClick={() => onNavigateView('VSL')} />
          </div>
        </section>

        <section className="border border-[#333] bg-[#080808] p-3">
          <div className="text-[#F27D26] text-xs font-bold mb-3">ROUTE / PORT SNAPSHOT</div>
          <div className="flex items-center gap-2 text-white text-sm font-bold mb-3"><span>{lane.originPort}</span><ArrowRight className="w-4 h-4 text-[#F27D26]" /><span>{lane.destinationPort}</span></div>
          <div className="grid grid-cols-2 gap-2 text-[10px]">
            <Stat label="DISTANCE" value={`${lane.distanceNm.toLocaleString()} NM`} />
            <Stat label="TRANSIT" value={`${lane.transitDays} DAYS`} />
            <Stat label="LANE CONGESTION" value={`${lane.congestionIndex}/100`} />
            <Stat label="DEST. WAIT" value={destination ? `${destination.avgWaitDays.toFixed(1)} DAYS` : 'N/A'} />
            <Stat label="VESSEL SUPPLY" value={`${lane.activeVessels}`} />
            <Stat label="CARBON EST." value={`$${lane.carbonEtsCostEst.toFixed(0)}`} />
          </div>
          <button onClick={() => onNavigateView('PORT')} className="w-full mt-3 border border-[#444] hover:border-[#F27D26] text-[#F27D26] py-2 text-[10px] font-bold">OPEN EAST COAST PORT INTELLIGENCE</button>
        </section>
      </div>

      <section className="border border-[#333] bg-[#080808] p-3 mb-3">
        <div className="flex items-center justify-between mb-3"><div className="text-[#F27D26] text-xs font-bold">MODEL EXPLAINABILITY</div><span className="text-[9px] text-gray-500">{forecast.modelName} • PROTOTYPE</span></div>
        <div className="grid md:grid-cols-4 gap-2">
          {forecast.featureContributions.map(item => <div key={item.feature} className="bg-[#111] border border-[#222] p-2"><div className="text-[9px] text-gray-500">{item.feature.toUpperCase()}</div><div className={item.impact >= 0 ? 'text-[#00FF41] font-bold mt-1' : 'text-red-400 font-bold mt-1'}>{item.impact >= 0 ? '+' : ''}{item.impact.toFixed(2)}</div></div>)}
        </div>
        <div className="text-[9px] text-gray-500 mt-3">Training samples: {forecast.metrics.samples} • MAE {forecast.metrics.mae.toFixed(1)} • RMSE {forecast.metrics.rmse.toFixed(1)} • MAPE {forecast.metrics.mape.toFixed(1)}%</div>
      </section>

      <div className="grid md:grid-cols-3 gap-2">
        <QuickLink icon={<PackageSearch className="w-4 h-4" />} title="PROCUREMENT" text="Compare overseas origins and buying window" onClick={() => onNavigateView('PROC')} />
        <QuickLink icon={<Ship className="w-4 h-4" />} title="CHARTERING" text="Match vessel capacity, timing and risk" onClick={() => onNavigateView('CHART')} />
        <QuickLink icon={<Anchor className="w-4 h-4" />} title="PORT INTEL" text="Inspect East Coast India constraints" onClick={() => onNavigateView('PORT')} />
      </div>
      <div className="text-[9px] text-gray-600 mt-3">DEMO / DECISION-SUPPORT DATA • Forecast is a prototype regression model, not a live market prediction or booking recommendation.</div>
    </div>
  );
};
