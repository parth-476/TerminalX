...
const vesselTypeForLane: Record<FreightLane['type'], Vessel['type']> = {
  DRY_BULK: 'DRY_BULK', CRUDE: 'CRUDE_TANKER', LNG: 'LNG_CARRIER', CONTAINER: 'CONTAINER'
};
...
export function buildCharterRecommendation(request: CharterRequest, lanes: FreightLane[], ports: Port[], vessels: Vessel[]): CharterRecommendation | null {
  ...
  const rationale = [
    `${lane.code} ML freight model: ${forecast.direction} (${forecast.change30dPct >= 0 ? '+' : ''}${forecast.change30dPct.toFixed(1)}% over 30D).`,
    `Model confidence: ${forecast.confidence.toFixed(0)}%; volatility: ${forecast.volatilityPct.toFixed(1)}%.`,
