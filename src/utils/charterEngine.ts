...
export function buildCharterRecommendation(request: CharterRequest, lanes: FreightLane[], ports: Port[], vessels: Vessel[]): CharterRecommendation | null {
  const lane = lanes.find(item => item.id === request.laneId); if (!lane) return null;
  ...
  const matches = vessels.filter(v => v.type === targetType).map(vessel => {
    ...
    const reasons = [capacityFit >= 100 ? 'capacity covers cargo + margin' : `capacity below ideal (${Math.round(capacityFit)}%)`];
    if (timingFit >= 75) reasons.push('laycan timing fit'); if (vessel.congestionWaitHours > 48) reasons.push(`${Math.round(vessel.congestionWaitHours)}h congestion wait`); if (vessel.riskAlert) reasons.push(`risk: ${vessel.riskAlert}`);
    return { vessel, score, capacityFit, timingFit, portFit, riskScore, reasons };
  }).filter(m => m.capacityFit >= 70).sort((a,b)=>b.score-a.score).slice(0,5);
  ...
  const rationale = [`${lane.code} ML forecast: ${forecast.direction} (${forecast.change30dPct >= 0 ? '+' : ''}${forecast.change30dPct.toFixed(1)}% over 30D).`, `Forecast confidence: ${forecast.confidence.toFixed(0)}%; MAPE ${forecast.metrics.mape.toFixed(2)}%.`, `Combined route/port congestion risk: ${congestionRisk}/100.`, matches.length ? `${matches[0].vessel.name} ranks first at ${matches[0].score}/100.` : 'No vessel currently meets the capacity screen.'];
