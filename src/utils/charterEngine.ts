...
export interface CharterRecommendation {
  ...
  lane: FreightLane;
  forecast: ReturnType<typeof forecastFreight>;
  originPort?: Port;
  destinationPort?: Port;
  matches: VesselMatch[];
  congestionRisk: number;
  decision: 'FIX NOW' | 'WAIT' | 'WATCH';
  rationale: string[];
}

const vesselTypeForLane: Record<FreightLane['type'], Vessel['type']> = {
  DRY_BULK: 'DRY_BULK',
  CRUDE: 'CRUDE_TANKER',
  LNG: 'LNG_CARRIER',
  CONTAINER: 'CONTAINER'
...
export function buildCharterRecommendation(
  request: CharterRequest,
  lanes: FreightLane[],
  ports: Port[],
  vessels: Vessel[]
): CharterRecommendation | null {
  const lane = lanes.find(item => item.id === request.laneId);
  if (!lane) return null;

  const forecast = forecastFreight(lane, 30);
  ...
  const matches = vessels
    .filter(vessel => vessel.type === targetType)
    .map(vessel => {
      const capacityFit = Math.min(100, (vessel.dwt / requiredDwt) * 100);
      const timingFit = Math.max(0, Math.min(100, 100 - Math.max(0, vessel.congestionWaitHours - request.laycanDays * 24) * 1.5));
      ...
      if (capacityFit >= 100) reasons.push('capacity covers cargo + margin');
      else reasons.push(`capacity below ideal (${Math.round(capacityFit)}%)`);
      if (timingFit >= 75) reasons.push('laycan timing fit');
    ...
    .filter(match => match.capacityFit >= 70)
    .sort((a, b) => b.score - a.score)
    .slice(0, 5);

  const decision: CharterRecommendation['decision'] =
    forecast.direction === 'UP' && congestionRisk >= 65 ? 'FIX NOW' :
    forecast.direction === 'DOWN' && congestionRisk < 70 ? 'WAIT' : 'WATCH';

  const rationale = [
    `${lane.code} freight model: ${forecast.direction} (${forecast.changePct >= 0 ? '+' : ''}${forecast.changePct.toFixed(1)}% over 30D).`,
    `Combined route/port congestion risk: ${congestionRisk}/100.`,
    matches.length ? `${matches[0].vessel.name} ranks first at ${matches[0].score}/100.` : 'No vessel currently meets the capacity screen.'
  ];
