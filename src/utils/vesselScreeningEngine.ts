...
const cargoType = (lane: FreightLane): VesselType => lane.type === 'DRY_BULK' ? 'DRY_BULK' : lane.type === 'CRUDE' ? 'CRUDE_TANKER' : lane.type === 'LNG' ? 'LNG_CARRIER' : 'CONTAINER';

const etaDaysFromNow = (eta: string, fallbackHours: number): number => {
  const parsed = Date.parse(eta.replace(' UTC', 'Z'));
  if (Number.isFinite(parsed)) return Math.max(0, (parsed - Date.now()) / 86400000);
  return Math.max(0, fallbackHours / 24);
};

export function screenVessels(lane: FreightLane, vessels: Vessel[], quantityMt: number, laycanDays = 14): VesselScreenResult[] {
  const requiredDwt = Math.max(quantityMt * 1.08, 1);
  ...
  return vessels.map(vessel => {
    ...
    const etaDays = etaDaysFromNow(vessel.eta, vessel.congestionWaitHours);
    ...
    rationale.push(`ETA fit ${etaDays.toFixed(1)}d vs ${laycanDays}d laycan`);
