...
export interface IncidentHotspot {
  id: string;
  title: string;
  category: 'SINKING' | 'MISSILE_ATTACK' | 'COLLISION' | 'OIL_SPILL' | 'PIRACY' | 'DROUGHT' | 'SEA_MINE' | 'SEVERE_WEATHER';
  vesselName: string;
  vesselType?: string;
  flag?: string;
  imo?: string;
  ...
  status: 'ACTIVE_HAZARD' | 'SALVAGE_IN_PROGRESS' | 'RESTRICTED_ZONE' | 'RESOLVED_MONITORING';
  environmentalImpact: string;
  insuranceImpact: string;
  freightImpact: string;
  detailedSitrep: string;
...
export interface FreightLane {
  ...
  code: string;
  name: string;
  type: 'CONTAINER' | 'DRY_BULK' | 'CRUDE' | 'LNG';
  originPort: string;
  destinationPort: string;
  ...
  rateUnit: string; // e.g. $/FEU, $/day (TCE), WS (Worldscale)
  change24h: number;
  changePct: number;
  ...
  carbonEtsCostEst: number; // USD per voyage
...
export interface NewsStory {
  ...
  category: 'FREIGHT' | 'MACRO' | 'COMMODITIES' | 'CENTRAL_BANKS' | 'GEOPOLITICAL';
  sentiment: 'BULLISH' | 'BEARISH' | 'NEUTRAL';
  summary: string;
...
export interface DealProposal {
  ...
  buyer: string;
  seller: string;
  status: 'PROPOSED' | 'COUNTERED' | 'CONFIRMED' | 'SETTLED' | 'CANCELLED';
...
export interface ChatChannel {
  id: string;
  name: string;
  type: 'DIRECT' | 'DESK_GROUP' | 'MARKET_CHAT';
  firm: string;
...
export interface OrderTicket {
  ...
  ticker: string;
  side: 'BUY' | 'SELL';
  type: 'LIMIT' | 'MARKET' | 'STOP' | 'ICEBERG' | 'TWAP';
  quantity: number;
  limitPrice?: number;
...
export interface Order {
  ...
  filledQty: number;
  avgFillPrice: number;
  status: 'NEW' | 'WORKING' | 'FILLED' | 'CANCELLED' | 'REJECTED';
  placedTime: string;
  filledTime?: string;
  algoParams?: string;
  venue: string;
